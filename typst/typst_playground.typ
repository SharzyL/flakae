#import "templates/modernism-report.typ": *

#show: modernism-report()

#make-title("Example Report", [Sharzy])

For any $k, x$, denote by $v_(k, x)$ the probability that A wins B by $x$ scores on round $k$.

Now $ p_(2m + 1) & = sum_(x >= 2) v_(2m + 1, x) = sum_(x >= 3) v_(2m + 1, x) \
...
Similarly $ q_(2m + 1) = q_(2m) - binom(2m, m + 1) p^(m) q^(m + 1). $
